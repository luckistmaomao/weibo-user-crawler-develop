...
def parse_user_profile(html, weibo_user_type=1001):
    ...
    if "identity_img" in html and 'company' in html:
        user_info.weibo_user_type = 1003
        weibo_user_type = 1003
    elif "identity_img" in html:
    ...
    for script in soup.findAll('script'):
        ...
        if 'FM.view(' in text:
            ...
            elif weibo_user_type == 1003:
                td = inner_soup.findAll('td', attrs={'class': 'S_line1'}) 
                if len(td) == 3:
                    try:
                        user_info.n_followees = int(td[0].strong.text.strip())
                        user_info.n_followers = int(td[1].strong.text.strip())
                        user_info.n_mblogs = int(td[2].strong.text.strip())
                    except Exception, e:
                        print traceback.format_exc()
                        raise UnsuspectedPageStructError
